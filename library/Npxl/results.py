...
def get_trial_accuracy(true, pred, pred_mask, trial):
    """
    Computes accuracy rate for a given trial.

    Args:
        true: true positions of a given trial
        pred: predicted positions of a given trial
        pred_mask: mask of predicted positions of a given trial excluding NaNs
    """
...
def get_trial_errors(true, pred, pred_mask, trial):
    """
    Computes absolute errors for a given trial.
...
def get_trial_wt_error(true, posterior, trial, num_tbins, num_pbins):
    """
    Computes mean weighted error for a given trial.

    Args:
        true: true positions of a given trial        
        posterior: posterior probabilities of a given trial
        num_tbins: number of time bins
        num_pbins: number of position bins
    """
    wt_errors_sum = []
...
def get_pred_pos_count(true, pred, pos):
    """
    Args:
        true: true positions of the whole session
        pred: predicted positions of the whole session
        pos: true position bin index

    Returns:
        pred_pos_count: Counter object of predicted positions for given true positions
        num_pred: total number of predictions for givne true position
    """
