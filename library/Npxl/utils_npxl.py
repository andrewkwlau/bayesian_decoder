...
def get_firstx_pos(mouse, position_mtx: np.ndarray, x: int) -> np.ndarray:
    """
    Find first x positions of each trial in the session.

    Args:
        mouse (d.NpxlData):
            NpxlData object containing the start locations of the trials.
    ...
    """
    num_trials = position_mtx.shape[0]
    ...
    for trial in range(num_trials):
        ...
        # Find the correct start location that is closest to the first position bin
        closest_start = int(mouse.start_locations[np.argmin(np.abs(mouse.start_locations - first_pos))])
        # store first 5 positions of the trial
        firstx_pos[trial] = np.array(range(closest_start, closest_start + x))
...
def create_spikesmask(
    ...
    if rewardzone is None:
        pass
    # Else, mask every position bin in list.
